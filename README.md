...
修改 `script.js` 或 `research-scene.js` 后运行 `npm ci` 和 `npm run build`，并同时提交 `assets/` 构建结果。HTML 和 CSS 修改不需要构建。依赖已固定版本，页面运行时不依赖外部 CDN。
...
- 首屏四种三维概念模型支持鼠标拖动、双指旋转、暂停播放和视角复位；移动端单指仍可滚动页面。不代表具体晶体结构、计算轨迹或反应模拟。
- 莫兰迪低饱和配色使用柔石墨色首屏、灰蓝色碳利用、暖灰色光、雾紫色等离子体和鼠尾草绿数据；氧原子为藕粉色，正文保持浅色阅读背景。
- 三维模型采用中性柔光和偏哑光材质，减弱金属高光与自发光；模型、示意图和控件共享 CSS 配色变量。
- Light 展示多层催化晶格、入射光子和分离的电子/空穴路径；Plasma 展示透明放电管、环形电极、催化剂床与运动粒子；Carbon 展示 CO2 接近、吸附弯曲和离开活性位点；Data 展示四层网络、连接和信息传递。
- 三维代码独立在 `research-scene.js`，通过 Three.js 实例化绘制重复晶格和粒子，使用 OrbitControls 管理视角。控件支持键盘，并在 WebGL 不可用时自动禁用。
